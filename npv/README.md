...
- `claude/输入数据需求清单.md`：跑通三个 Python 版本所需的输入文件、JSON 字段、六张表与运行参数的逐项说明。
...
- `claude/权威度偏序标注prompt设计.md`：LLM 偏序标注的输入选择、两两比较与绝对档位 prompt、落库规则。
- `claude/特征探索与新特征建议.md`：现有特征逐个拆解、五类判错、四层新特征清单与筛选流程。
- `claude/权威度调研/页面本身信号的业界做法调研.md`：无点击数据时业界如何从页面本身判权威与质量。
